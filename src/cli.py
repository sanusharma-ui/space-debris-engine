...
def run_cli():
    ...
    use_tle = input("\nUse real TLE data? (y/N): ").strip().lower().startswith("y")
    ...
    if use_tle:
        ...
        norad_sat = int(input("NORAD ID of satellite (e.g. 25544 for ISS): "))

        # single reference epoch for this entire run (IMPORTANT)
        epoch0 = datetime.now(timezone.utc)

        name_s, tle1_s, tle2_s = fetch_tle(norad_sat)

        # pass same epoch
        r_s, v_s = tle_to_state(tle1_s, tle2_s, epoch=epoch0)
        satellite = entity_from_tle(r_s, v_s)

        # attach TLE metadata for SGP4 screening
        satellite.name = getattr(satellite, "name", None) or name_s or f"NORAD-{norad_sat}"
        satellite.norad_id = int(norad_sat)
        satellite.tle1 = tle1_s
        satellite.tle2 = tle2_s
        satellite.epoch_utc = epoch0          # ✅ add this
        satellite.satrec = satrec_from_tle(tle1_s, tle2_s)  # ✅ cache satrec
        satellite.frame = "TEME"


        # -------- LOAD MULTIPLE DEBRIS FROM TXT FILE --------
        debris_list = []

        try:
            with open("debris_ids.txt", "r") as f:
                debris_ids = [
                    int(line.strip())
                    for line in f
            ...
            print("⚠️ debris_ids.txt not found — no debris loaded from file.")
            debris_ids = []

        print(f"Loaded {len(debris_ids)} debris IDs from file")
        ...
        for did in debris_ids:
            try:
                ...
                # pass same epoch
                r_d, v_d = tle_to_state(tle1_d, tle2_d, epoch=epoch0)
                debris = entity_from_tle(r_d, v_d)
                debris.name = f"NORAD-{did}"

                debris.norad_id = int(did)
                debris.tle1 = tle1_d
                debris.tle2 = tle2_d
                debris.epoch_utc = epoch0                  # ✅ add this
                debris.satrec = satrec_from_tle(tle1_d, tle2_d)  # ✅ cache satrec
                debris.frame = "TEME"

                debris_list.append(debris)

            except Exception as e:
                print(f"⚠️ Skipping debris {did}: {e}")
