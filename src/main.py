...
#         if mode == "engine2":
#             # direct high-fidelity run (already correct)
...
#                 e2r = engine2.run(sat_entity, deb_entity, duration=float(lookahead), use_engine1_escalation=False)
...
#                     sat_entity = Entity(position=satellite.position, velocity=satellite.velocity)
#                     deb_entity = Entity(position=debris_obj.position, velocity=debris_obj.velocity)
#                     mc = engine2.run_monte_carlo(sat_entity, deb_entity, duration=float(lookahead), N=Nval)
#                     mc_results[did] = mc
